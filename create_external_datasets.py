...
def main():
    ...
    output_dir = Path("/Users/rodri/Desktop/TabLLM/datasets_serialized") / temp_output
    if not args.debug:
        os.mkdir(output_dir)
    ...
    if args.debug:
        ...
        dataset["validation"] = dataset["validation"].sample(
            min(5, len(dataset["validation"]))
        )
            ...
            for column_idx, c in enumerate(dataset.columns):
                ...
                if column_idx not in cat_idx and c != "label":
                    ...
                    for bin_idx in range(bin_num):
                        lower_idx, upper_idx = math.floor(bin_idx * factor), math.floor(
                            (bin_idx + 1) * factor
                        )
                        bin_idx_intervals.append([lower_idx, upper_idx])
                        bin_idx_endpoints.append(
                            [
                                sorted_value_list[lower_idx],
                                sorted_value_list[upper_idx - 1],
                            ]
                        )

                    for value in value_list:
                        for bin_idx, (lower_value, upper_value) in enumerate(
                            bin_idx_endpoints
                        ):
                            if value >= lower_value and value <= upper_value:
                                mapped_bin_lower_idx, mapped_bin_upper_idx = (
                                    bin_idx_intervals[derangement[bin_idx]]
                                )
                                sampled_bin_values = sorted_value_list[
                                    mapped_bin_lower_idx:mapped_bin_upper_idx
                                ]
    ...
    if args.tabletotext or args.t0serialization:
        ...
        if args.tabletotext:
            ...
            def table_to_text(note):
                ...
                examples = [
                    write_into_table(x[0].strip(), x[1].strip()) for x in name_values
                ]
                return [preprocess(e)["linearized_table"] for e in examples]

            # notes = notes[0:10]
            old_size = len(notes)
            notes = Dataset.from_dict(
                {"text": list(itertools.chain(*[table_to_text(n) for n in notes]))}
            )
            ...
            notes = [
                (((ex["out"].split(">")[-2]).split("<")[0]).replace("\n", " ")).strip()
                for ex in notes
            ]
            notes = [
                " ".join(l)
                for l in [
                    notes[x : x + num_features]
                    for x in range(0, len(notes), num_features)
                ]
            ]
        ...
        if args.t0serialization:
            ...
            def entry_to_text(note):
                ...
                chunks = [
                    (prefix + ", ".join(lines[k : k + 2]) + suffix)
                    for k in range(0, len(lines), 2)
                ]
                return chunks

            old_size = len(notes)
            num_chunks = int(((len(dataset.columns) - 1) / 2.0) + 0.5)
            # notes = notes[0:10]
            notes = Dataset.from_dict(
                {"text": list(itertools.chain(*[entry_to_text(n) for n in notes]))}
            )
            ...
            notes = [
                " ".join(l)
                for l in [
                    notes[x : x + num_chunks] for x in range(0, len(notes), num_chunks)
                ]
            ]
...
def output_linear_classifier_features(examples, output_dir, dataset):
    ...
    scalings = {
        ...
        "car": {},
        "heart": {
            "Age": [53.63760218, 9.38893213],
            "RestingBP": [132.09264305, 18.09209337],
            "Cholesterol": [201.70844687, 107.50566557],
            "FastingBS": [0.23160763, 0.42185962],
            "MaxHR": [136.59945504, 25.12828773],
            "Oldpeak": [0.92711172, 1.06128969],
        },
        "diabetes": {
            "Pregnancies": [3.68403909, 3.28025968],
            "Glucose": [120.41042345, 32.63939221],
            "BloodPressure": [68.75081433, 19.83518715],
            "SkinThickness": [20.22638436, 15.68020872],
            "Insulin": [79.43485342, 114.8289827],
            "BMI": [31.77654723, 8.02507088],
            "DiabetesPedigreeFunction": [0.47113192, 0.33090205],
            "Age": [32.90879479, 11.66936554],
        },
    }
...
def create_perturbed_income_examples(examples, output_dir):
    ...
    prob_feature_perturbed = 2.0 / 12  # on average two features perturbed
    ...
    for idx, ex in examples.iterrows():
        for p in range(0, num_perturbed_examples_per_example):
            perturbed_feature_mask = (
                np.random.uniform(0, 1, 12) < prob_feature_perturbed
            )
            ...
            for f, feat in enumerate(list(feature_values.keys())):
                if perturbed_feature_mask[f]:
                    ...
                    if isinstance(feature_values[feat], int):
                        example_copy[feat] = int(
                            random.uniform(0, feature_values[feat])
                        )
                    else:
                        example_copy[feat] = feature_values[feat][
                            int(random.uniform(0, len(feature_values[feat])))
                        ]
    ...
    for column in [
        c for c in examples_dummies.columns if c not in example_variants_dummies.columns
    ]:
    ...
    assert (
        counter == example_variants_dummies.shape[0]
        and counter == example_variants.shape[0]
    )
