...
def post_request(example, model, yes_no_probability=False):
    ...
    if model == "gpt3":
        ...
        data = (
            '{"prompt": "'
            + text
            + '", "temperature": 0, "max_tokens": 1, "logprobs": 50}'
        )
        ...
        if "error" in response_json:
            if not response_json["error"]["message"].startswith(
                "Rate limit reached"
            ):  # Ignore rate limit error.
                raise Exception(
                    "ERROR: " + response_json["error"]["message"] + " " + text
                )

        if yes_no_probability:
            logprobs = response_json["choices"][0]["logprobs"]["top_logprobs"][0]
            yes_prob = (
                0 if " Yes" not in logprobs.keys() else math.exp(logprobs[" Yes"])
            )
...
def read_dataset(task, input_file):
    ...
    if task in public_tasks:
        ...
        temp = [
            ...
            if t.get_name() == templates_for_custom_tasks[task]
        ][0]
        input_list = [
            {"note": temp.apply(x)[0], "answer": temp.apply(x)[1], "label": x["label"]}
            for x in orig_data
        ]
    else:
        ...
        try:
            ...
            for i, prompt_temp in enumerate(config["prompts"]):
                ...
                if args.model == "gpt3":
                    ...
                    time.sleep(0)
                outputs = pd.concat(
                    [outputs, pd.Series(output).to_frame(1).T], ignore_index=True
                )

                if args.model == "gpt3" and k % 50 == 0:
                    # Write temporary results out
                    outputs.to_csv(
                        "output/outputs-" + args.task + start_time + ".csv", index=False
                    )
